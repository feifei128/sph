<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="item in address" :key="item.id" @click="changeDefault(item)">
      ...
      <!-- 订单信息 -->
      <div class="detail" v-for="good in detailArrayList" :key="good.id">
        <h5>商品清单</h5>
    ...
    <!-- 总计 + 地址 -->
    <div class="trade">
      <div class="price">应付金额: <span>¥{{trade.totalAmount}}</span></div>
...
export default {
  ...
  computed: {
    ...mapGetters(['address', 'detailArrayList', 'trade']),
    // 将选中的收货信息保存下来，在提交订单按钮之前进行渲染
    userDefaultAddress() {
      return this.address.find(ele => ele.isDefault === '1') || []
    }
  },
  mounted() {
  ...
  methods: {
    ...
    async submitOrder() {
      ...
      try {
        await this.$store.dispatch('getSubmitOrder', { tradeNo, data })
      } catch (error) {
        alert('提交订单失败：' + error.massage)
      }
    }
  }
}
